...
场景一：resolve 在 then 之后执行（此时数组有值）

```javascript
const p = new MyPromise((resolve) => {
    setTimeout(() => {
        resolve(100); // 1秒后才执行，晚于 then
    }, 1000);
});

p.then((val) => console.log(val)); // 这一步先执行
```


场景二：resolve 在 then 之前执行（此时数组为空）

```javascript
const p = new MyPromise((resolve) => {
    resolve(100); // 同步立即执行，早于 then
});

p.then((val) => console.log(val)); // 这一步后执行
```
